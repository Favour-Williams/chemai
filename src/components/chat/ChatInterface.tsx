...
const ChatInterface: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  // Fix: Select theme and updateTheme individually to prevent infinite loop
  const theme = useSettingsStore((state) => state.theme);
  const updateTheme = useSettingsStore((state) => state.updateTheme);
  
  // Derive isDark from theme and create toggleTheme function
 ...
 useEffect(() => {
    if (isOpen) {
      try {
        ...
        error('LLM Service Error', err.message);
      }
    }
  }, [isOpen]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiErrorMessage = err.message || 'Failed to get AI response';
  ...
  const handleVoiceInput = async () => {
    ...
    try {
      setIsRecording(true);
      setAudioLevel(0.5); // Simulate audio level
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-6xl h-[500px] rounded-lg shadow-2xl flex overflow-hidden">
              ...
              {conversations.map((conv) => (
                <div key={conv.id} className="relative group">
                  <button
                    ...
                    className={`w-full p-3 rounded-lg text-left transition-colors pr-10 ${
                  ...
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity" data-chat-menu>
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        setShowChatMenu(showChatMenu === conv.id ? null : conv.id);
                      }}
                    ...
                    {showChatMenu === conv.id && (
                      <div 
                        className={`absolute top-full right-0 mt-1 w-32 rounded-md shadow-lg border z-50 ${
                          isDark ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-200'
                        }`}
                        data-chat-menu
                      >
                        <button
                          ...
                          className={`w-full px-3 py-2 text-left text-sm rounded-md transition-colors flex items-center space-x-2 ${
                            isDark ? 'text-red-400 hover:bg-gray-600' : 'text-red-600 hover:bg-gray-50'
                          }`}
                        >
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-1">
                {chatService.isLLMAvailable() ? (
                  <Zap className="h-4 w-4 text-green-500" title="LLM Connected" />
                ) : (
                  <ZapOff className="h-4 w-4 text-yellow-500" title="Using Fallback Responses" />
                )}
                <span className={`text-xs ${
                  chatService.isLLMAvailable() ? 'text-green-500' : 'text-yellow-500'
                }`}>
                  {chatService.isLLMAvailable() ? chatService.getLLMConfig()?.provider : 'Fallback'}
                </span>
              </div>
            </div>
            <div className="flex items-center space-x-2">
          ...
          <div className={`flex-1 overflow-y-auto p-6 space-y-4 ${
            ...
            {messages.length === 0 && (
              <div className="text-center py-8">
                ...
                {!chatService.isLLMAvailable() && (
                  <div className={`mt-4 p-3 rounded-lg ${isDark ? 'bg-yellow-900/20 text-yellow-300' : 'bg-yellow-50 text-yellow-800'}`}>
                    <p className="text-sm">
                      💡 Add an LLM API key to your .env file for real AI responses
                    </p>
                  </div>
                )}
              </div>
              ...
              <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className="group relative max-w-[80%]">
                  ...
                  <div className={`absolute top-0 ${
                    message.sender === 'user' ? 'left-0 transform -translate-x-full' : 'right-0 transform translate-x-full'
                  } opacity-0 group-hover:opacity-100 transition-opacity flex items-center space-x-1 px-2`}>
                    <button
                      onClick={() => handleCopyMessage(message.text, message.id)}
                      className={`p-1.5 rounded-lg transition-colors ${
                        isDark ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-200 text-gray-600'
                      }`}
                      title={copySuccess === message.id ? 'Copied!' : 'Copy message'}
                    >
                    ...
                    {message.sender === 'ai' && voiceService.isSupported().synthesis && (
                      <button
                        onClick={() => voiceService.speak(message.text)}
                        className={`p-1.5 rounded-lg transition-colors ${
                          isDark ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-200 text-gray-600'
                        }`}
                        title="Read aloud"
          ...
          <div className={`p-4 border-t ${
            ...
            <div className="flex space-x-3">
              ...
              >
                ...
                {isRecording && (
                  <div className="absolute inset-0 rounded-lg overflow-hidden">
                    <div
                      className="absolute bottom-0 left-0 right-0 bg-red-400 transition-all duration-100"
                      style={{ height: `${audioLevel * 100}%`, opacity: 0.3 }}
                    />
                  </div>
                )}
              </button>
              <input
                ...
                className={`flex-1 px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 transition-colors ${
                  isDark
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-lg transition-colors ${
                  inputText.trim() && !isRecording && !isTyping
